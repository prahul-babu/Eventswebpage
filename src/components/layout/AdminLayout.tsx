    ...
    <RequireAuth allowedRoles={["admin"]}>
      <div className="min-h-screen flex flex-col bg-slate-50/60 text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <main className="flex-1">
          <ErrorBoundary fallbackTitle="Admin Console Notice">
            <Outlet />
          </ErrorBoundary>
        </main>
        <footer className="border-t bg-white py-6 mt-12 text-slate-500 text-xs">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span>&copy; {new Date().getFullYear()} The Apollo University. Institutional Administration.</span>
            </div>
            <div className="flex items-center gap-2 text-slate-400">
              <ShieldCheck className="h-3.5 w-3.5 text-amber-500" />
              <span>Global Administrator Console</span>
            </div>
          </div>
        </footer>
      </div>
